...
def detArgs( args, parser ):
	if (args.printout == True):
		if (args.files != None or args.algorithm != None or args.specifier != None or args.location != None):
			...
			path = ['V:\\Congo\\DIVA\\test\\', 'V:\\Congo\\DIVA\\test\\']
			ext = ['tar', 'tar']
			loc = 'V:\\Congo\\DIVA\\test\\'
...
def getHash( file, alg='md5' ):
	...
	if (alg == 'md5'):
		...
		hash = sha1Checksum(file)
	elif (alg == 'sha224'):
		hash = sha224Checksum(file)
	elif (alg == 'sha256'):
		hash = sha256Checksum(file)
	elif (alg == 'sha384'):
		hash = sha384Checksum(file)
	elif (alg == 'sha512'):
		hash = sha512Checksum(file)
	else:
		raise ArgumentException('Error: unknown hash algorithm')
	return hash

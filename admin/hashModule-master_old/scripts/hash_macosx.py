...
def detArgs( args, parser ):
	...
	elif (args.samma == True):
		samma.samma_macosx.main(args, parser)
...
def multiPrompts( arry=None, algorithm=None, specification=None, location=None, distRend=False ):
	'''
	...
	if (distRend == True):
		...
		if (specification == None):
			...
			if (multiSpecs == True):
				for name in filename:
					...
					tempspec = getInput(concat)
					if (tempspec == 'show'):
						list = hashcreate.listPrintFunc()
						chsnspec, printbool = showListPrompt(list)
						spec.append(chsnspec)
					elif (tempspec == '' or tempspec == None):
						spec.append('default')
						printbool = False
					else:
						printbool = False
					while printbool == True:		
						...
						hashcreate.main(hash[0], False, path[0], name, ext[0], alg[0], None, None, None, None, spec, printbool)
		...
		if (specification == None):
			...
			if (multiSpecs == True):
					...
					if (location == None):
						...
						if (altloc == True):
							...
							try:
								...
								hashcreate.main(hash[count], False, path[count], name, ext[count], alg[count], None, None, None, None, spec, printbool, loc)
								print "\nYour file has been created and saved to: " + loc
						else:
							hashcreate.main(hash[count], False, path[count], name, ext[count], alg[count], None, None, None, None, spec, printbool)
							print "\nYour file has been created and saved to: " + path[count]
					elif (location == 'path'):
						hashcreate.main(hash, False, path, filename, ext, alg, None, None, None, None, spec, printbool)
				...
				while printbool == True:		
					print "\nSpecifier chosen: " + spec
					hashcreate.main(hash[0], False, path[0], filename[0], ext[0], alg[0], None, None, None, None, spec, printbool)
					newmsg = "Is this the format you would like to use?"
					loopbool = yOrN(newmsg)
					if (loopbool == True):
						printbool = False
				...
				if (location == None):
					...
					if (altloc == True):
						...
						try:
							...
							hashcreate.main(hash, True, path, filename, ext, alg, None, None, None, None, spec, printbool, loc)
							print "\nYour files have been created and saved to: " + loc
					else:
						hashcreate.main(hash, True, path, filename, ext, alg, None, None, None, None, spec, printbool)
			...
			if (location == None):
				...
				if (altloc == True):
					...
					try:
						...
						hashcreate.main(hash, True, path, filename, ext, alg, None, None, None, None, spec, False, loc)
						print "\nYour files have been created and saved to: " + loc
				else:
					hashcreate.main(hash, True, path, filename, ext, alg, None, None, None, None, spec, False)
					print "\nYour files have been created and saved to: " + path[0]
			elif (location == 'path'):
				hashcreate.main(hash, False, path, filename, ext, alg, None, None, None, None, spec, False)
				print "\nYour file has been created and saved to: " + path
			else:
				loc = location
				hashcreate.main(hash, True, path, filename, ext, alg, None, None, None, None, spec, False, loc)
...
def multiNoPrompts( arry, algorithm=None, specification=None, location=None, distRend=False ):
	files = []
	for f in arry:
		files.append(f)
	hash = []
...
def singlePrompts( file=None, algorithm=None, specification=None, location=None, distRend=False ):
	'''
	...
	if (location == None):
		...
		if (altloc == True):
			...
			try:
				...
				hashcreate.main(hash, False, path, filename, ext, alg, None, None, None, None, spec, printbool, loc)
				print "\nYour file has been created and saved to: " + loc
		else:
			hashcreate.main(hash, False, path, filename, ext, alg, None, None, None, None, spec, printbool)
			print "\nYour file has been created and saved to: " + path
...
def singleNoPrompts( file, algorithm=None, specification=None, location=None, distRend=False ):
	'''
	...
	if (location == None):
		loc = None
		hashcreate.main(hash, False, path, filename, ext, alg, None, None, None, None, spec, False, loc)
	else:
		loc = location[0]
		loc = loc.rstrip()
		hashcreate.main(hash, False, path, filename, ext, alg, None, None, None, None, spec, False, loc)

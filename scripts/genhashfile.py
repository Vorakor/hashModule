...
########################################################################################
def main( hashObject ): #hash, bool, path, filename, ext, alg, filesize=None, filetype=None, fileduration=None, filemuxrate=None, specifier='default', printbool=False, loc=None ):
	if(hashObject.specifier == 'diva'):
		if (hashObject.loc != None):
			diva(hashObject): #hash, bool, path, filename, ext, alg, printbool, loc)
		else:
			diva(hashObject): #hash, bool, path, filename, ext, alg, printbool)
	elif(hashObject.specifier == 'samma'):
		if (hashObject.loc != None):
			samma(hashObject): #hash, bool, path, files, ext, alg, filesize, filetype, fileduration, filemuxrate, printbool, loc)
		else:
			samma(hashObject): #hash, bool, path, files, ext, alg, filesize, filetype, fileduration, filemuxrate, printbool)
	else:
		if (hashObject.loc != None):
			default(hashObject): #hash, bool, path, filename, ext, alg, printbool, loc)
		else:
			default(hashObject): #hash, bool, path, filename, ext, alg, printbool)
	return
...
def genExamplePrint( sampleObject ): #spec, details, arry ):
	msg = 'This is what the ' + sampleObject.spec + ' file will look like when it is created and printed:\n'
	if (sampleObject.spec == 'diva'):
		printStatement(sampleObject)#msg, arry, details)
	# Add your own elif statements below this point
	elif (sampleObject.spec == 'samma'):
		printSammaStatement(sampleObject)#msg, arry, details)
# 		print msg
# 		for i in arry:
# 			i = i.replace('\n', '')
# 			print i
